...
function createZip(files) {
  ...
  for (const file of files) {
    const nameBuffer = Buffer.from(normalizeName(file.name), "utf8");
    const inputBuffer = Buffer.isBuffer(file.content)
      ? file.content
      : Buffer.from(String(file.content || ""), "utf8");
    const compressed = zlib.deflateRawSync(inputBuffer);
