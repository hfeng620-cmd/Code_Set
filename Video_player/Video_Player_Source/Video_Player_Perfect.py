...
class ClickableSlider(QSlider):
    ...
    def mouseMoveEvent(self, event):
        """鼠标移动事件 - 显示悬停预览和处理拖动"""
        # 显示时间预览（仅在未拖动滑块时显示）
        if self.maximum() > 0 and self.time_formatter and not self._dragging:
            x_pos = event.pos().x()
            # 计算鼠标位置对应的进度值（线性映射）
            if self.width() > 0:
                value = self.minimum() + (self.maximum() - self.minimum()) * x_pos / self.width()
        ...
        # 如果在拖动中，实时更新位置
        if self._dragging and self._position_setter and self.maximum() > 0:
            # 计算拖动位置对应的值
            x_pos = event.pos().x()
            value = self.minimum() + (self.maximum() - self.minimum()) * x_pos / self.width()
            value = max(self.minimum(), min(self.maximum(), int(value)))
...
class ClickableVideoWidget(QVideoWidget):
    def __init__(self, parent=None):
        ...
        self.seek_callback = None  # 快进/快退回调函数
        self._last_mouse_pos = None  # 记录上次鼠标位置
        self._dragging = False  # 拖动状态标志
    
    def mousePressEvent(self, event):
        """处理鼠标点击事件 - 点击视频画面切换播放/暂停，开始拖动"""
        if event.button() == Qt.LeftButton:
    ...
    def mouseMoveEvent(self, event):
        """处理鼠标移动事件 - 转发给鼠标移动回调，处理拖动快进/快退"""
        # 如果有鼠标移动回调，调用它（用于全屏模式控制条显示）
        if self.mouse_move_callback:
            self.mouse_move_callback(event)
        
        # 处理拖动快进/快退
        if self._dragging and self._last_mouse_pos is not None and self.seek_callback:
            ...
            if abs(delta_x) > 5:
                # 计算快进/快退的时间（毫秒）
                # 每10像素对应1秒
                seek_ms = int(delta_x * 100)  # 100毫秒/像素
                self.seek_callback(seek_ms)
                # 更新上次位置，避免连续触发
...
class FullscreenPlayer(QWidget):
    def __init__(self, media_player, parent=None):
        ...
        # 绑定快进/快退回调
        if parent and hasattr(parent, 'quick_seek'):
            self.video_widget.set_seek_callback(parent.quick_seek)
        ...
        """)
        # 确保控制条容器不拦截鼠标事件
        self.controls_container.setAttribute(Qt.WA_TransparentForMouseEvents)
...
class AudioVideoPlayer(QWidget):
    def __init__(self):
        ...
        # 0. 窗口调整标志
        self._first_video_loaded = False  # 标记是否是第一次加载视频
        self._last_window_size = None  # 记录上次窗口大小（全屏前）
        self._last_window_position = None  # 记录上次窗口位置（全屏前）
        ...
        try:
            ...
            # 读取所有视频的播放位置（字典：文件路径 -> 播放位置）
            playback_positions_str = settings.value('playback_positions', '{}')
            try:
                self.playback_positions = json.loads(playback_positions_str)
                # 确保所有值都是整数
                for key, value in self.playback_positions.items():
                    self.playback_positions[key] = int(value)
            except:
    ...
    def set_window_icon(self):
        """设置窗口图标 - 应用图标使用 favicon.ico"""
        # 图标文件路径列表（按优先级尝试）
        icon_paths_to_try = [
            # 1. 资源目录中的 favicon.ico（开发环境）- 更新为新的路径
            r"D:\VS_Code\Code_Set\Video_player\Video_Player_Resources\favicon.ico",
            # 2. 打包后的资源目录
            os.path.join(os.path.dirname(sys.executable), "Video_Player_Resources", "favicon.ico"),
            # 3. 当前目录（开发环境备用）
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "favicon.ico"),
            # 4. 其他可能的位置
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "nap_cn.ico"),
        ]
        
        icon_path = None
        for icon_path_candidate in icon_paths_to_try:
            if os.path.isfile(icon_path_candidate):
        ...
        if icon_path:
            try:
                ...
                # 如果设置失败，尝试使用可执行文件内置的图标
                self._set_default_icon()
        else:
            print("⚠️ 未找到应用图标文件，将使用可执行文件内置图标")
            self._set_default_icon()
    ...
    def _set_default_icon(self):
        ...
        try:
            # 尝试使用可执行文件本身作为图标源
            # 在Windows上，可执行文件通常包含图标资源
            if getattr(sys, 'frozen', False):
                # 打包后的可执行文件
                exe_path = sys.executable
                self.setWindowIcon(QIcon(exe_path))
                print(f"✅ 使用可执行文件内置图标: {exe_path}")
            else:
                # 开发环境，尝试使用当前脚本目录中的图标
                ...
                for icon_path in possible_icons:
                    if os.path.isfile(icon_path):
                        self.setWindowIcon(QIcon(icon_path))
                        print(f"✅ 使用开发环境图标: {icon_path}")
                        return
                print("⚠️ 未找到任何图标文件，使用系统默认图标")
        except Exception as e:
            print(f"⚠️ 设置默认图标失败: {e}")

    def init_ui(self):
        # 初始化用户界面（图标已在 set_window_icon 方法中设置）

        # 媒体组件
        self.mediaPlayer = QMediaPlayer(None, QMediaPlayer.VideoSurface)
        self.video_widget = ClickableVideoWidget()  # 使用可点击的视频窗口
        self.video_widget.player_callback = self.play_video  # 绑定点击暂停回调
        self.video_widget.set_seek_callback(self.quick_seek)  # 绑定拖动快进/快退回调
        
        # 设置视频缩放模式：保持纵横比，窗口自适应视频尺寸
        self.video_widget.setAspectRatioMode(Qt.KeepAspectRatio)
        
        # 设置视频窗口的大小策略，确保它扩展以填充可用空间
        self.video_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        
        # 连接视频尺寸变化信号，实现窗口自适应（添加异常处理）
        try:
            self.mediaPlayer.videoAvailableChanged.connect(self.on_video_available_changed)
        except Exception:
            # 如果信号不可用，忽略错误
            pass
    ...
    def play_specific_file(self, file_path):
        # 在切换文件前，先保存当前文件的播放位置
        if self.current_playing_file and self.current_playing_file != file_path:
        ...
        # 恢复该文件的播放位置
        self.restore_file_position(file_path)
    ...
    def show_video_list_menu(self, position):
        ...
        menu = QMenu(self)
        # 添加查看视频地址选项
        view_path_action = menu.addAction("查看视频地址")
        view_path_action.triggered.connect(lambda: self.view_video_path(item))
        menu.addSeparator()
    ...
    def show_recent_list_menu(self, position):
        ...
        # 添加查看视频地址选项
        view_path_action = menu.addAction("查看视频地址")
        view_path_action.triggered.connect(lambda: self.view_video_path(item, is_recent=True))
        menu.addSeparator()
        rename_action = menu.addAction("重命名视频文件")
    ...
    def view_video_path(self, item, is_recent=False):
        """查看视频文件的完整路径"""
        if item is None:
            return
        ...
        # 添加复制按钮
        copy_button = msg_box.addButton("复制路径", QMessageBox.ActionRole)
        ...
        if msg_box.clickedButton() == copy_button:
            ...
            self.label.setText(f"已复制路径到剪贴板: {filename}")
    ...
    def save_playback_position(self):
        ...
        try:
            if self._settings is not None and self.current_playing_file:
                ...
                # 保存到播放位置字典
                self.playback_positions[self.current_playing_file] = position
                # 保存整个字典到设置
                self._settings.setValue('playback_positions', json.dumps(self.playback_positions))
                # 同时保存当前播放文件（用于恢复上次播放）
                self._settings.setValue('last_played_file', self.current_playing_file)
        except Exception:
            pass
    ...
    def restore_last_position(self):
        ...
        try:
            if (hasattr(self, 'last_played_file') and self.last_played_file and 
                ...
                # 从字典中获取该文件的播放位置
                saved_position = self.playback_positions.get(self.last_played_file, 0)
                
                # 恢复播放位置
                if saved_position > 0:
                    # 连接durationChanged信号，确保在媒体加载完成后设置位置
                    def on_duration_changed(duration):
                        if duration > 0 and saved_position > 0:
                            # 确保位置不超过总时长
                            pos = min(saved_position, duration)
    ...
    def restore_file_position(self, file_path):
        """恢复特定文件的播放位置"""
        try:
            if file_path and os.path.isfile(file_path):
                # 从字典中获取该文件的播放位置
                saved_position = self.playback_positions.get(file_path, 0)
    ...
    def set_playback_rate(self, rate):
        ...
        if rate == 1.0:
            ...
            self.label.setText(f"播放速度: {rate}x (音调会变化，这是正常现象)")
            
            # 在全屏模式下显示提示信息
            if self.is_fullscreen and self.fullscreen_player:
                hint_text = f"播放速度: {rate}x (音调变化正常)"
                self.fullscreen_player.show_hint(hint_text)
    ...
    def enter_fullscreen(self):
        ...
        self._last_window_position = self.pos()
    ...
    def exit_fullscreen(self):
        ...
        # 恢复之前保存的窗口大小和位置
        if self._last_window_size and self._last_window_position:
            self.resize(self._last_window_size)
            self.move(self._last_window_position)
        ...
        if duration > 0:
            ...
            total_time = self.format_time(duration)
            self.timeLabel.setText(f"{current_time} / {total_time}")
        self.label.setText("已退出全屏，窗口已恢复原大小")
    ...
    def quick_seek(self, offset_ms):
        ...
        # 在全屏模式下显示提示信息
        if self.is_fullscreen and self.fullscreen_player:
            hint_text = f"{direction} {duration_sec} 秒"
            self.fullscreen_player.show_hint(hint_text)
    ...
    def on_video_available_changed(self, available):
        """视频可用性变化时调整窗口大小"""
        if available:
            # 只在第一次加载视频时调整窗口大小
            if not self._first_video_loaded:
                self._first_video_loaded = True
                # 延迟调整窗口大小，确保视频已加载
                QTimer.singleShot(100, self.adjust_window_to_video)
    
    def adjust_window_to_video(self):
        """根据视频尺寸调整窗口大小（只在窗口不是最大化状态时调整）"""
        # 检查窗口是否最大化
        if self.isMaximized():
            # 如果窗口已经最大化，不进行调整
            return
        ...
        # 计算视频区域的理想大小（考虑控制条和列表区域）
        # 视频区域应该占据窗口的主要部分
        ideal_video_width = 800  # 默认视频宽度
        ideal_video_height = 450  # 默认视频高度（16:9比例）
        
        # 计算窗口总大小（包括列表区域和控制条）
        list_width = 200  # 列表区域宽度
        control_height = 50  # 控制条高度
        
        new_width = list_width + ideal_video_width + 20  # 加上边距
        new_height = ideal_video_height + control_height + 40  # 加上边距
        
        # 确保窗口不会太小
        new_width = max(new_width, 800)
        new_height = max(new_height, 600)
        
        # 如果当前窗口大小与理想大小相差较大，调整窗口
        if abs(current_width - new_width) > 100 or abs(current_height - new_height) > 100:
            self.resize(new_width, new_height)
            self.label.setText(f"窗口已调整为: {new_width}x{new_height}")
